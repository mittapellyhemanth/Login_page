...
UserRegister.post("/register", async (req, res) => {
    ...
    if (!password || password.length < minLength || !specialCharacters.test(password)) {
        return res.status(400).json({ error: "Password must be at least eight characters long and contain at least one special character" });
    }
